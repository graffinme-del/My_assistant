...
def looks_like_cancel_court_sync_jobs(text: str) -> bool:
    ...
    if re.search(r"(?:отчет|отчёт)\s+(?:по\s+)?(?:задач[еаи])", lowered):
        return False
    if re.search(r"\b(?:не|не\s+надо|не\s+нужно|не\s+следует)\s+(?:\w+\s+){0,3}(?:останови|остановить|отмени|сними|снять|сбрось|очисти|удали|убери)\b", lowered):
        return False

    polite_prefix = r"^(?:(?:пожалуйста|пж|можешь|можно|срочно|давай|ну)[,\s]+)*"
    action = r"(?:сними|снять|отмени|отменить|останови|остановить|сбрось|сбросить|очисти|очистить|удали|удалить|убери|убрать)"
    target = r"(?:задач\w*|процесс\w*|очеред\w*|скачиван\w*|загрузк\w*|фон\w*|кад\b)"
    if re.search(rf"{polite_prefix}\b{action}\b.*\b{target}", lowered):
        return True
    return bool(re.search(rf"{polite_prefix}\b(?:останови|остановить)\b\s+(?:все|всё)\s*[.!?]*$", lowered))
