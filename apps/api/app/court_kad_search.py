...
def looks_like_cancel_court_sync_jobs(text: str) -> bool:
    ...
    command = re.sub(r"\s+", " ", lowered).strip(" \t\r\n.!?,;:«»\"'()[]{}")
    if re.search(r"(?:отчет|отчёт)\s+(?:по\s+)?(?:задач[еаи])", lowered):
        return False
    if not command or re.match(r"^(?:не|не надо|не нужно)\b", command):
        return False
    polite = r"(?:пожалуйста\s+)?"
    return any(
        re.match(pattern, command)
        for pattern in (
            rf"^{polite}останови\b.*\b(?:процесс|процессы|задач|скачиван|загрузк|фонов)",
            rf"^{polite}останови\s+(?:всё|все)$",
            rf"^{polite}остановить\b.*\b(?:задач|фонов)",
            rf"^{polite}отмени\b.*\b(?:задач|скачиван|загрузк|кад)",
            rf"^{polite}сними\b.*\bзадач",
            rf"^{polite}снять\b.*\bзадач",
            rf"^{polite}сбрось\s+очередь(?:\s+кад)?$",
            rf"^{polite}очисти\s+очередь(?:\s+кад)?$",
            rf"^{polite}удали\s+старые\s+задачи$",
            rf"^{polite}убери\s+дубли\s+задач$",
        )
    )
