...
async def llm_system_user(system: str, user: str, *, timeout: float = 120.0) -> str:
    ...
    try:
        ...
        if code == 401:
            raise ValueError(
                "OpenAI/OpenRouter вернул 401: проверьте OPENAI_API_KEY и OPENAI_BASE_URL."
            ) from e
        raise
    data = resp.json()
    choices = data.get("choices") or []
...
async def llm_assistant_chat_reply(user_message: str, case: Case, *, prompt_override: str | None = None) -> str:
    ...
    user_content = prompt_override or (
        ...
        "Не пиши шаблонных фраз вроде 'могу помочь', если действие уже можно выполнить. "
        "Не вставляй сырые технические логи, stack trace и длинные URL; ответ — короткие абзацы простым языком, как в обычном мессенджере.\n\n"
