...
from .calendar_month_quarter import (
...
def handle_delete_documents_chat(
    ...
    if month_or_quarter_blocks_bulk_document_mutation(text, explicit_document_ids=doc_ids):
        return (
            DELETE_REFUSED_MONTH_QUARTER,
            fallback_case,
        )
...
async def assistant_ingest_text(
    ...
    if looks_like_move_all_from_active_case_to_folder(text):
        if month_or_quarter_blocks_bulk_document_mutation(text):
            unsorted_case = get_or_create_unsorted_case(db)
            return await finalize_reply(
                case=unsorted_case,
                reply_text=MOVE_REFUSED_MONTH_QUARTER,
                mode="documents-bulk-move-month-quarter-refused",
            )
