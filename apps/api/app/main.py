...
def handle_delete_documents_chat(
    ...
    if holiday_blocks_bulk_document_mutation(text, explicit_document_ids=doc_ids):
        return (
            DELETE_REFUSED_HOLIDAY,
            fallback_case,
        )
...
async def assistant_ingest_text(
    ...
    if looks_like_move_all_from_active_case_to_folder(text):
        if holiday_blocks_bulk_document_mutation(text):
            unsorted_case = get_or_create_unsorted_case(db)
            return await finalize_reply(
                case=unsorted_case,
                reply_text=MOVE_REFUSED_HOLIDAY,
                mode="documents-bulk-move-holiday-refused",
            )
