...
def handle_delete_documents_chat(
    ...
    if looks_like_clock_or_slash_iso_date_scoped_document_request(text):
        return (
            "Не удаляю файлы по времени или дате вроде «15:00» / «15/03/2026» / «15 числа» — это не номер документа. "
            "Укажите id: «удали документ 214» или «удали документы [12] [18]». "
            "Чтобы очистить открытую папку целиком, напишите «удали все документы в этой папке» без указания даты или времени.",
            fallback_case,
        )
...
def handle_court_sync_chat_command(
    ...
    *,
    ...
    m_job = re.search(
        ...
        r"(?:(?:отчет|отчёт|статус)\s+(?:по\s+)?)?задач[еаи]\s*(?:#|№)?\s*(?:номер\s*)?(\d+)",
        lowered,
    )
    if m_job:
