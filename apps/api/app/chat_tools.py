...
CHAT_TOOLS: list[dict[str, Any]] = [
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "Не вызывай, если пользователь просит открыть/показать **конкретный** уже известный файл по числовому id "
                "или по **полному имени файла** с расширением («документ 254», «файл [254]», «документ отчёт.pdf») — "
                "такие запросы обрабатывает сервер без семантического поиска. "
                "Не вызывай, если пользователь просит сохранить текст самого сообщения в папку как заметку. "
                "В queries передай 1–10 фраз без служебных слов («найди», «документы»): ФИО, фамилии судей, номер дела, "
                "редкие названия. При **нескольких** фразах сервер отдаёт документы, где совпало **несколько** критериев "
                "(или одна длинная фраза), чтобы отсечь чужие файлы; одно короткое слово вроде «банк» само по себе мало что даст."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    ...
                    "queries": {
                        ...
                        "maxItems": 10,
                        "description": (
                            "Поисковые фразы. Несколько фраз — для строгого отбора нужны совпадения по нескольким или длинная фраза."
                        ),
                    },
                },
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Удалить лишние копии одного и того же файла в разных папках: остаётся одна копия. "
                "По умолчанию группа = одинаковое имя **и** совпадающий распознанный текст PDF "
                "(разные «Определение.pdf» из разных дел не трогаются). "
                "Модель/эвристика выбирает, какую копию оставить. "
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Объединить папки автоматически только при нескольких подтверждённых дубликатах "
                "(одинаковое имя файла **и** совпадающий текст PDF) между папками. "
                "Одно общее имя вроде «Определение.pdf» без совпадения текста не сливает дела. "
                "Не вызывай, если пользователь явно назвал две конкретные папки через «и» — тогда обычное объединение вручную."
            ),
            "parameters": {"type": "object", "properties": {}},
        },
    },
