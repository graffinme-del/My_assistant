...
def open_case_and_download_documents(
    ...
    *,
    ...
    """
    Если prebuilt_documents задан (например, из Parser-API), используем их как начальный
    список, но всё равно обходим КАД/Мой Арбитр: Parser-API может вернуть неполный набор.
    """
    nav_ms = max(60_000, MOY_ARBITR_TIMEOUT_SEC * 1000)
    ...
    try:
        ...
        if prebuilt_documents:
            for row in prebuilt_documents[:cap]:
                doc = dict(row)
                u = (doc.get("file_url") or "").strip()
            ...
            if progress and job_id is not None:
                progress(
                    job_id,
                    "opening_case",
                    f"Мой Арбитр: {len(docs)} документов из Parser-API, продолжаю обход КАД.",
                )
