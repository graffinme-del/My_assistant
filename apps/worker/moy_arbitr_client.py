...
def open_case_and_download_documents(
    ...
    *,
    ...
    дополнительный список ссылок, но всё равно обходим КАД/«Мой Арбитр»:
    Parser-API может вернуть неполный набор материалов.
    """
    nav_ms = max(60_000, MOY_ARBITR_TIMEOUT_SEC * 1000)
    ...
    try:
        ...
        if prebuilt_documents:
            before = len(docs)
            docs = merge_prebuilt_documents(docs, prebuilt_documents, cap=MOY_ARBITR_MAX_DOCS_PER_CASE)
