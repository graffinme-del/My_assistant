...
def _append_unique_documents(docs: list[dict], seen_fu: set[str], rows: list[dict]) -> None:
    for row in rows:
...
def open_case_and_download_documents(
    ...
    *,
    ...
    """
    Если prebuilt_documents задан (например, из Parser-API), используем его как стартовый список
    и всё равно добираем документы из вкладок КАД / хаба my.arbitr: Parser-API может вернуть
    неполный набор ссылок.
    """
    nav_ms = max(60_000, MOY_ARBITR_TIMEOUT_SEC * 1000)
    ...
    try:
        ...
        if prebuilt_documents:
            _append_unique_documents(docs, seen_fu, [dict(x) for x in prebuilt_documents])
            if progress and job_id is not None:
                progress(
                    job_id,
                    "opening_case",
                    f"Мой Арбитр: {len(docs)} документов из Parser-API, добираю материалы из КАД.",
                )
        ...
        if is_kad_card:
            ...
            if cn:
                for row in _collect_documents_via_my_arbitr_hub(
                    ...
                    _append_unique_documents(docs, seen_fu, [row])
                    if len(docs) >= MOY_ARBITR_MAX_DOCS_PER_CASE:
                        break
        else:
            _append_unique_documents(docs, seen_fu, collect_moy_arbitr_documents(page, card_url))
            try:
                ...
                _append_unique_documents(docs, seen_fu, extra)
            except Exception:
                pass
        if not docs:
            try:
