...
def open_case_and_download_documents(
    ...
    *,
    ...
    """
    Если prebuilt_documents задан (например, из Parser-API), используем их как стартовый список,
    но всё равно проверяем вкладки КАД/«Мой Арбитр», чтобы не потерять документы при частичном ответе API.
    """
    nav_ms = max(60_000, MOY_ARBITR_TIMEOUT_SEC * 1000)
    ...
    try:
        ...
        if prebuilt_documents:
            for item in prebuilt_documents[:cap]:
                row = dict(item)
            ...
            if progress and job_id is not None:
                progress(
                    job_id,
                    "opening_case",
                    f"Мой Арбитр: {len(docs)} документов из Parser-API, проверяю браузером дополнительные материалы.",
                )
        import worker as worker_mod
        ...
        if is_kad_card:
            try:
                browser_docs = worker_mod.open_kad_card_and_collect_docs(
                    page,
                    card_url.strip(),
                    nav_ms,
                    progress=progress,
