...
def open_case_and_download_documents(
    ...
    *,
    ...
    """
    prebuilt_documents (например, из Parser-API) используем как seed-список,
    но всё равно обходим браузерные источники: Parser-API может вернуть неполную карточку.
    """
    nav_ms = max(60_000, MOY_ARBITR_TIMEOUT_SEC * 1000)
    ...
    try:
        ...
        if prebuilt_documents:
            ...
            for row in prebuilt_documents[:cap]:
                u = ((row or {}).get("file_url") or "").strip()
